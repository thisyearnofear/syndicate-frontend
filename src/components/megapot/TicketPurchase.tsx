    ...
    onStatusChange: (status) => {
      if (status === "Executed") {
        toast.success("Tickets purchased successfully!", {
          description: `${ticketCount} ticket${ticketCount !== 1 ? 's' : ''} purchased for ${syndicateName}.`,
        });
      ...
      <div className="space-y-4">
        ...
        {isConfirming && (
          <div className="bg-cyan-900/30 border border-cyan-500/30 rounded-md p-3 text-sm">
            ...
            <p className="mb-2">You are buying {ticketCount} ticket{ticketCount !== 1 ? "s" : ""} for ${totalCost.toFixed(2)} GHO.</p>
            <p className="mb-1"><strong>{causePercentage}%</strong> of any winnings will go to <strong>{causeName}</strong>.</p>
            <p className="text-xs text-white/60">This is a cross-chain transaction from Lens Chain (GHO) to Base (USDC). Tickets will be purchased on the Base network.</p>
          </div>
        )}
