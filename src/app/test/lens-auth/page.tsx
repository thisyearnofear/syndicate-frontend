...
export default function LensAuthTestPage() {
  ...
  const { isAuthenticated, isLoading, authorizeResponse } = useLensAuthContext();
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <h1 className="text-3xl font-bold mb-8 text-center">Lens Protocol Authentication Test</h1>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
        <Card>
          <CardHeader>
            <CardTitle>Wallet Connection</CardTitle>
            <CardDescription>Connect your wallet to interact with Lens Protocol</CardDescription>
          </CardHeader>
          <CardContent>
            {isConnected ? (
              <div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Connect to Lens Protocol using your wallet</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="mb-4">
              <p className="text-sm mb-2">
                Status: <span className={isAuthenticated ? "text-green-500" : "text-yellow-500"}>
                  {isLoading ? "Connecting..." : isAuthenticated ? "Authenticated" : "Not Authenticated"}
                </span>
              </p>
              
              {isConnected && (
                <LensLoginButton className="w-full mb-4" />
              )}
              
              {isAuthenticated && authorizeResponse && (
                <div className="mt-4">
                  <p className="text-sm font-semibold mb-2">Authorization Details:</p>
